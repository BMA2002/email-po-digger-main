...
export async function scanFileForVirus(filePath: string): Promise<void> {
  try {
    ...
    await scanner.init({
      ...
      clamscan: {
        path: process.platform === 'win32' ? 'clamscan.exe' : '/usr/bin/clamscan', // Adjust path for Windows
        db: null,
        scan_archives: true,
        active: true
      },
      clamdscan: {
        ...
        timeout: 60000,
        local_fallback: true,
        path: process.platform === 'win32' ? 'clamdscan.exe' : '/usr/bin/clamdscan',
        config_file: null,
        multiscan: true,
        reload_db: false,
        active: true,
    ...
    if (error.message.includes('clamscan') || error.message.includes('clamdscan') || error.message.includes('command not found') || error.message.includes('not found')) {
      throw new Error('ClamAV is not installed or not found. Please install ClamAV to use virus scanning. On Windows, download from https://www.clamav.net/downloads and ensure it\'s in your PATH or specify the full path.');
    }
...
export async function scanFile(filePath: string): Promise<VirusScanResult> {
  try {
    ...
    await scanner.init({
      ...
      scan_recursively: false,
      clamscan: {
        path: process.platform === 'win32' ? 'clamscan.exe' : '/usr/bin/clamscan',
        db: null,
        scan_archives: true,
        active: true
      },
